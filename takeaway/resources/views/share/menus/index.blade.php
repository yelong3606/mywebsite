...
<div class="card mb-2" id="category{{$category_id}}">
    ...
    <table class="table table-bordered table-striped table-sm">
        ...
        <tr>
            <td class="col-8">
                <a
                    href="{{ route('menus.edit', ['shop' => $shop->id, 'menu' => $menu->id]) }}">{!!$menu->title!!}
                </a>
                @if (!$has_variants)
            ...
            <td class="col-2">
                <div class="btn-group">
                    <a class="btn btn-outline-secondary btn-sm"
                        href="{{ route('menus.edit', ['shop' => $shop->id, 'menu' => $menu->id]) }}">Edit</a>

                    <form
                        action="{{ route('menus.destroy', ['shop' => $shop->id, 'menu' => $menu->id]) }}"
                        method="POST">
                        @csrf
                        @method('DELETE')
